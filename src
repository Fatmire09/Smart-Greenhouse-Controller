...
const int STEPS_115 = 654;
const float TEMP_OPEN  = 25.0;
const float TEMP_CLOSE = 23.0;
...
void handleGreenhouse(float temperature) {
  if (!portaAperta && temperature > TEMP_OPEN) {
    setRGB(pumpState ? 1 : 0, pumpState ? 0 : 1, 0);
    Serial.println("OPENING GREENHOUSE");
    stepper.step(-STEPS_120);
    portaAperta = true;
  }
  else if (portaAperta && temperature < TEMP_CLOSE) {
    setRGB(pumpState ? 1 : 0, pumpState ? 0 : 1, 0);
